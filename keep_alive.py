...
def _safe_latency(bot) -> str:
    """Возвращает строку латентности, обрабатывая nan/None."""
    if bot is None:
        return "—"
    lat = bot.latency
    if lat is None or (isinstance(lat, float) and math.isnan(lat)):
        return "—"
    return str(round(lat * 1000, 1))
...
async def handle_home(request: web.Request) -> web.Response:
    ...
    try:
        _lat_num = float(latency)
    except (ValueError, TypeError):
        _lat_num = -1
    latency_color = "ok" if 0 <= _lat_num < 100 else "warn" if _lat_num < 300 else "error"
  ...
  <div class="main">
    ...
    <div class="card-grid">
      <div class="stat">
        ...
        <div class="stat-value" style="color:var(--{latency_color})">{latency + "ms" if latency != "—" else latency}</div>
